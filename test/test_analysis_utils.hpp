...
template<typename T>
bool are_pointer_sets_equal(const std::set<std::shared_ptr<T>> &set_1, const std::set<std::shared_ptr<T>> &set_2)
{
...
void test_elementary_blocks(bool print)
{
    ...
    print_test_result(are_pointer_sets_equal<Block>(res, expected), __func__);
}
...
void test_control_flow(bool print)
{
    ...
    print_test_result(are_sets_equal<std::pair<PP, PP>>(res, expected), __func__);
}
